...
public class RDFXMLHandler implements LiftingHandler, LoweringHandler {
    ...
    public void lower(Model model, OutputStream entityStream, MediaType mediaType, ResourceDescription presentation) throws ResourcePlatformException {
        if (presentation != null) {
            throw new ResourcePlatformException("Lowering handler RDFXMLHandler can only lower RDF content with no specified presentation");
        }
        if (!mediaType.isCompatible(MediaType.valueOf(MEDIA_TYPE))) {
            throw new ResourcePlatformException("Lowering handler RDFXMLHandler can only lower RDF content to a representation with MediaType " + MEDIA_TYPE);
        }
    ...
    public Model lift(MediaType mediaType, ResourceDescription presentation, InputStream entityStream) throws ResourcePlatformException {
        if (!mediaType.isCompatible(MediaType.valueOf(MEDIA_TYPE))) {
            throw new ResourcePlatformException("Lifting handler RDFXMLHandler can only lift representations with MediaType " + MEDIA_TYPE);
        }
